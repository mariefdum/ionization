...
# options.method = 'bdf'
# options.order = 5
# options.norm_tol = 0.00001

options = qt.Options(nsteps=1e9,atol=1e-8,rtol=1e-06,method='bdf',order=5)#,norm_tol=0.00001)
options = {'nsteps':1e9,'atol':1e-8,'rtol':1e-06,'method':'bdf','order':5}#,norm_tol=0.00001)
...
def generate_FA(Floquet_params,verbose=5):
    ...
    results = Parallel(n_jobs=num_cores,verbose=verbose)(delayed(floquet_eigs)(init,Floquet_params) for init in init_cons)
    
    results_index=[]
    for i in range(len(eps_d_vec)):
        results_index.append(results[i][0])
    results_f_eigs=[]
    for i in range(len(eps_d_vec)):
        results_f_eigs.append(results[i][1])
    index_tot=np.array(results_index)
    f_energies_tot=np.array(results_f_eigs)
    f_modes_0_tot=[]
    for i in range(len(eps_d_vec)):
        f_modes_0_tot.append(results[i][2])

    return results_index,f_energies_tot,f_modes_0_tot
...
def floquet_eigs(init,Floquet_params):  
    i=int(init[0])
    eps_d=init[1]
    ...
    f_modes_0,f_energies = qt.floquet_modes(H, T, args=Floquet_params ,options=options)
...
def track(q_max,f_energies,f_modes_0,Floquet_params,Transmon_params,eigs_t):
    ...
    for q in range(q_max):
        ...
        #f_modes_q.append(f_modes_0[0][q])
        #f_energies_q.append(f_energies[0,q])

        f_modes_q.append(qt.basis(trunc_charge,q))
        ...
        for j in range(len(eps_d_vec)-1):
            overlap_i=0
            index=0
            for i in range(trunc_charge):
                overlap=np.abs(f_modes_q[j].overlap(f_modes_0[j+1][i]))**2
                if overlap>overlap_i:
                    index_2=np.copy(index)
                    index=i
                    overlap_i_2=np.copy(overlap_i)
                    overlap_i=overlap
                    f_mode_next=f_modes_0[j+1][i]
            overlap_max_q.append(overlap_i)
            overlap_max_q_2.append(overlap_i_2)
            f_modes_q.append(f_mode_next)
            f_energies_q.append(f_energies[j+1,index])
            index_q.append(index)
            index_q_2.append(index_2)

        overlap_max.append(overlap_max_q)
        overlap_max_2.append(overlap_max_q_2)
        f_modes_0_sorted.append(f_modes_q)
        f_energies_sorted.append(f_energies_q)
        index_sorted.append(index_q)
        index_sorted_2.append(index_q_2)

    return overlap_max,f_modes_0_sorted,f_energies_sorted,index_sorted#,overlap_max_2,index_sorted_2
...
def generate_FA_time_av(index_sorted,Floquet_params,Transmon_params):
    ...
    i_vec=np.arange(0,len(eps_d_vec))
    init_cons=np.vstack((i_vec,eps_d_vec)).T
    results = Parallel(n_jobs=num_cores,verbose=3)(delayed(time_av_H)(init,Floquet_params,Transmon_params) for init in init_cons)
...
def time_av_H(init,Floquet_params,Transmon_params):
    ...
    f_modes_t = qt.floquet_modes_table(f_modes_0, f_energies, times, H, T, Floquet_params, options=options)
    
    expect_energy_transmon = np.zeros((trunc_charge, N_times))
    expect_energy = np.zeros((trunc_charge, N_times))
    N_t_all = np.zeros((trunc_charge, N_times))
    
    for n, t in enumerate(times):
        modes_t = qt.floquet_modes_t_lookup(f_modes_t, t, T)
        expect_energy_transmon[:, n] = [np.real(expect_t_transmon(mode, t,Floquet_params)) for mode in modes_t]
        expect_energy[:, n] = [np.real(expect_t(mode, t,Floquet_params,eps_d)) for mode in modes_t]
        N_t_all[:,n]  = [np.real(N_t(mode,Transmon_params)) for mode in modes_t]
    
    av_E_transmon = np.zeros((trunc_charge,))
    av_E = np.zeros((trunc_charge,))
    N_t_av = np.zeros((trunc_charge,))

    for n in range(trunc_charge):
...
def N_t(mode,Transmon_params):
    trunc_charge=Transmon_params['trunc_charge']
    return (np.squeeze(np.abs(mode[:])**2)).dot(np.arange(0,trunc_charge))
...
def N_t_min(q,sweep_param_vec,q_list_tot,index_match_list_tot,f_modes_0_sorted_tot,Transmon_params):
    N_t_min=[]
    for sweep_param_index,sweep_param in enumerate(sweep_param_vec):   
        N_t_min_index=[]
        ...
        for index_q,new_q in enumerate(q_list):
            ...
            N_t_min_index.append(N_t(f_modes_0_sorted_tot[sweep_param_index][new_q],Transmon_params)[index_i:index_f])
...
def N_t_min_nosweep(q,q_list_tot,index_match_list_tot,f_modes_0_sorted_tot,Transmon_params):
    N_t_min_index=[]
    q_list=q_list_tot[0]
    ...
    for index_q,q in enumerate(q_list):
        index_i=index_match_list[index_q]
        index_f=index_match_list[index_q+1]  
        N_t_min_index.append(N_t(f_modes_0_sorted_tot[q],Transmon_params)[index_i:index_f])
...
def plot_quasi_energies(f_energies_ordered,eps_d_vec,trunc_charge,omega_r,text='yes',xscale='n'):
    q_max_plot=trunc_charge
    
    for q in range(q_max):
        f_energies_ordered_q=np.array(f_energies_ordered[q])
        x_i=0
        for i in range(len(f_energies_ordered_q)-1):
            if np.abs((f_energies_ordered_q[i]-f_energies_ordered_q[i+1]))>0.9*omega_r:
                x_f=i
                x=(eps_d_vec[x_i:x_f]/2/g)
                y=np.array((f_energies_ordered[q])[x_i:x_f])/2/np.pi/10**9
                if xscale=='n':
                    plt.plot(x**2,y,'-',markersize=1,linewidth=0.5,label=q,color=palette[q%len(palette)])
                elif xscale=='eps_d':
                    plt.plot(x,y,'-',markersize=1,linewidth=0.5,label=q,color=palette[q%len(palette)])
                x_i=i+1
        x=(eps_d_vec[x_i:]/2/g)
...
def calculate_overlap_tot(n_trunc,f_modes_0_sorted,eps_d_vec):
    ...
    for eps_d_index,eps_d in enumerate(eps_d_vec):
        if eps_d_index<len(eps_d_vec)-100:
...
def find_resonance_overlaps(n_g,n_trunc,eps_d_vec,f_energies_sorted,N_t_sorted,overlap_max,overlap_tot,param_resonances):
                ...
                if has_local_extrema(N_t_sorted[q0][n-FWHM:n+FWHM]):
                    if has_local_extrema(N_t_sorted[q1][n-FWHM:n+FWHM]):
                        if N_t_sorted_q0<N_t_sorted_q1:
                            if np.all(N_t_sorted[q0][n-FWHM:n+FWHM]<mean) or np.all(N_t_sorted[q1][n-FWHM:n+FWHM]>mean):
                                swap_statement=False
                        if N_t_sorted_q0>N_t_sorted_q1:
                            if np.all(N_t_sorted[q0][n-FWHM:n+FWHM]>mean) or np.all(N_t_sorted[q0][n-FWHM:n+FWHM]<mean):                    
                                swap_statement=False        
        
                index_min=np.max([n-FWHM,0])
                index_max=np.min([n+FWHM,len(N_t_sorted[q0])])
                comp=N_t_sorted[q0][index_min:index_max]-N_t_sorted[q1][index_min:index_max]
...
def find_resonances_N_t(q_array,f_modes_0_sorted,Transmon_params,Floquet_params,y_threshold=0.001,distance_max=50):
    trunc_charge=Transmon_params['trunc_charge']
    eps_d_vec=Floquet_params['eps_d_vec']
    ...
    for add in range(0,distance_max):
        for i in range(len(index_der_tot_array)):
            ...
            for j in range(len(index_der_tot_array)):
                ...
                if j>i:
                    if index_der_tot_array[i][1]==index_der_tot_array[j][1]+add and index_der_tot_array[i][1]>0 and index_der_tot_array[j][1]>0:
...
def gap(omega_d,eps_d_vec,index_match_list,q_list,f_energies_sorted):
    ...
    for index_match_i,index_match in enumerate(index_match_list[0]):
        if index_match>0 and index_match<len(eps_d_vec) :
            ...
            gap_1=np.abs(f_energies_sorted[q_f][index_match]-f_energies_sorted[q_i][index_match])
            gap_2=omega_d-np.abs(f_energies_sorted[q_f][index_match]-f_energies_sorted[q_i][index_match])
